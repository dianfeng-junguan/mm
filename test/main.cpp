...
void test_slab_lifecycle() {
  ...
  // 2. Try to allocate one more, should fail from this slab and create a new
  // one
  ...
  assert(cache.slabs_partial != NULL);
  assert(PTRLIST_NEXT(cache.slabs_partial) !=
         NULL); // Should have two partial slabs now
...
void test_mm_canary() {
  printf("\n--- Test: MM Allocator with Canary ---\n");

#ifndef NO_GLOBAL_SLAB_CACHE_ARRAY
  // Test 1: Normal allocation and free
  printf("  Sub-test: Normal alloc/free\n");
  void *p1 = mm_malloc(200, 8);
  assert(p1 != NULL);
  strcpy((char *)p1, "hello world");
  printf("    Allocated p1: %p, content: %s\n", p1, (char *)p1);
  mm_free(p1);
  printf("    Freed p1. OK.\n");

  // Test 2: Buffer overflow detection
  printf("  Sub-test: Buffer overflow detection\n");
  char *p2 = (char *)mm_malloc(50, 1);
  assert(p2 != NULL);
  printf("    Allocated p2: %p\n", (void *)p2);
  // Corrupt the memory right after the user block to overwrite the canary
  printf("    Writing past the allocated boundary to corrupt canary...\n");
  for (int i = 0; i <= 50 + 5; i++) { // overflow
    p2[i] = 'A';
  }
  printf("    Freeing p2. Expecting a corruption message...\n");
  mm_free(p2); // This should print a corruption message.
  printf("    Corruption check finished.\n");

  // Test 3: Realloc
  printf("  Sub-test: Realloc\n");
  void *p3 = mm_malloc(30, 8);
  assert(p3 != NULL);
  strcpy((char *)p3, "short string");
  printf("    Allocated p3: %p, content: '%s'\n", p3, (char *)p3);

  void *p4 = mm_realloc(p3, 100, 8);
  assert(p4 != NULL);
  printf("    Reallocated to p4: %p\n", p4);
  // The old content should be preserved
  assert(strcmp((char *)p4, "short string") == 0);
  printf("    Content after realloc: '%s'\n", (char *)p4);
  strcat((char *)p4, " and now it is a much longer string");
  printf("    New content: '%s'\n", (char *)p4);
  mm_free(p4);
  printf("    Freed p4. OK.\n");

  // Test 4: Realloc with no previous allocation
  printf("  Sub-test: Realloc with NULL pointer\n");
  void *p5 = mm_realloc(NULL, 70, 8);
  assert(p5 != NULL);
  strcpy((char *)p5, "newly allocated via realloc");
  printf("    Allocated p5 via realloc(NULL): %p\n", p5);
  printf("    Content: '%s'\n", (char *)p5);
  mm_free(p5);
  printf("    Freed p5. OK.\n");

#else
  printf("Skipping mm allocator tests because NO_GLOBAL_SLAB_CACHE_ARRAY is "
         "defined.\n");
#endif
  printf("MM Allocator test PASSED.\n");
}
